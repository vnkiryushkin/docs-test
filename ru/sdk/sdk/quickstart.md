...
- .NET {#dotnet}
    ...
    1. Склонируйте репозиторий с примерами {{ yandex-cloud }} SDK для .NET:

        ```bash
        git clone https://github.com/yandex-cloud-examples/yc-sdk-quickstart-dotnet.git
        ```

    1. В склонированном репозитории перейдите в директорию `compute/create-instance`:
...
- .NET {#dotnet}
    ...
    Где:
    * `AUTH_KEY` — путь к файлу с [авторизованным ключом](../../iam/concepts/authorization/key.md), который вы получили во время [подготовки рабочей директории](#prepare-project);
